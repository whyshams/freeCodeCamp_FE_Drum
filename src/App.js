...
return (
  <button value="test" id={keyCode} className="drum-pad btn btn-warning p-3 m-2" onClick={() => play(key, id)}>
...
const DumControle = ({ stop, name, power, volume, handleVolumeChange, changeSoundGroup }) => (
  <div className="controle">
    <button className='btn btn-danger' onClick={stop}>Turn Power {power ? 'OFF' : 'ON'}</button>
    <h2 className='m-5'>Volume: %{Math.round(volume * 100)}</h2>
    <input
      max="1"
      min="0"
      step='0.01'
      ...
      />
    <h2 className='mt-5' id="display" >{name}</h2>
    <button className='btn btn-primary m-1 p-3' onClick={changeSoundGroup}>Change Sounds Group</button>
  </div>
);
